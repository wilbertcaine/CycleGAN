...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        for idx, (X, Y) in enumerate(loop):
            ...
            if idx % 200 == 0:
                name = config.NAME
                save_image(X * 0.5 + 0.5, f"saved_images_{name}/X_{epoch}_{idx}.png")
                save_image(fake_X * 0.5 + 0.5, f"saved_images_{name}/fake_X_{epoch}_{idx}.png")
                save_image(rec_X * 0.5 + 0.5, f"saved_images_{name}/rec_X_{epoch}_{idx}.png")
                save_image(Y * 0.5 + 0.5, f"saved_images_{name}/Y_{epoch}_{idx}.png")
                save_image(fake_Y * 0.5 + 0.5, f"saved_images_{name}/fake_Y_{epoch}_{idx}.png")
                save_image(rec_Y * 0.5 + 0.5, f"saved_images_{name}/rec_Y_{epoch}_{idx}.png")

            loop.set_postfix(X_real=X_reals / (idx + 1), X_fake=X_fakes / (idx + 1))

        if epoch % 5 == 0 and config.SAVE_MODEL:
            name = config.NAME
            save_checkpoint(F, opt_gen, filename=f"saved_images_{NAME}/f_{epoch}.pth")
            save_checkpoint(G, opt_gen, filename=f"saved_images_{NAME}/g_{epoch}.pth")
            save_checkpoint(D_X, opt_disc, filename=f"saved_images_{NAME}/d_x_{epoch}.pth")
            save_checkpoint(D_Y, opt_disc, filename=f"saved_images_{NAME}/d_y_{epoch}.pth")
